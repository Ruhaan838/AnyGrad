...
def _use_utils_ops(operation_name, dtype, **kwargs):
    ...
    try:
        operation_func = getattr(C, f"{operation_name.capitalize()}{dtype_mapping[dtype].capitalize()}")
    except Exception as e:
        pass
    
    if 'low' in kwargs:
        data, shape = operation_func(kwargs['shape'], kwargs['low'], kwargs['high'], kwargs['generator'])
    else:
...
def rand(*shape, generator=None, requires_grad=False, dtype=Th.float32):
    if dtype not in {Th.float32, Th.float64}:
        raise TypeError("In valid dtype is provide for the rand operation. use float32 and float64")
...
def randint(*shape, low=0, high, generator=None, requires_grad=False, dtype=Th.int32):
    if dtype not in {Th.int32, Th.int64}:
        raise TypeError("Invalid dtype is provide for the randint operation. use int32 and int64")
    if generator is None:
        generator = C.GeneratorBase(random.randint(0, 100))
    return _use_utils_ops(
        shape=shape,
